...
function AuthPage() {
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="hidden bg-gradient-to-br from-primary via-primary to-primary/70 p-12 text-primary-foreground lg:flex lg:flex-col lg:justify-between">
      ...
      <div className="flex items-center justify-center bg-background p-6 sm:p-12">
        <div className="w-full max-w-md">
          <Link to="/" className="mb-8 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground lg:hidden">
            <GraduationCap className="h-5 w-5" /> Kembali ke beranda
          </Link>
          <h2 className="font-display text-2xl font-bold text-foreground">Masuk ke Admin</h2>
          <p className="mt-1 text-sm text-muted-foreground">Hanya pengelola sekolah.</p>
          ...
          <Tabs defaultValue="signin" className="mt-6">
            ...
            <TabsContent value="signin">
              <form onSubmit={handleSignIn} className="mt-4 space-y-4">
                <Field icon={Mail} type="email" label="Email" value={email} onChange={setEmail} placeholder="admin@sekolah.id" />
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignUp} className="mt-4 space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Nama admin" required />
                </div>
                <Field icon={Mail} type="email" label="Email" value={email} onChange={setEmail} placeholder="admin@sekolah.id" />
                <Field icon={Lock} type="password" label="Password" value={password} onChange={setPassword} placeholder="Minimal 6 karakter" />
                <Button type="submit" disabled={loading} className="w-full bg-primary text-primary-foreground">
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Buat Akun"}
                </Button>
                <p className="text-xs text-muted-foreground">Akun pertama otomatis jadi admin. Akun berikutnya perlu diberi role admin oleh super admin.</p>
              </form>
            </TabsContent>
          </Tabs>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <Input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} required className="pl-9" />
      </div>
    </div>
  );
}
